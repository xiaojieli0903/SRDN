...
local utils_fast = require 'super_resolution.utils_fast'
local models_fast = require 'super_resolution.models_fast'

local gm = require 'graphicsmagick'
local c = require 'trepl.colorize'
local cmd = torch.CmdLine()
-- Generic options
--cmd:option('-arch','c9s1-32,d64,d128,R128,R128,R128,R128,R128,u64,u32,u32,u16,c9s1-3')
---- 'c9s1-32,d64,d128,R128,R128,R128,R128,R128,U2,c3s1-64,U2,c3s1-32,c9s1-3')
cmd:option('-arch','c3s1-64,c5s1-128,R128,R128,R128,R128,R128,U2,c3s1-64,U2,c3s1-32,c9s1-3')-- 'c9s1-32,d64,d128,R128,R128,R128,R128,R128,U2,c3s1-64,U2,c3s1-32,c9s1-3')
cmd:option('-use_instance_norm', 0)
--cmd:option('-task', 'style', 'style|upsample')
--cmd:option('-h5_file', 'data/ms-coco-256.h5')
cmd:option('-padding_type', 'zero')
...
cmd:option('-val_output','./val_train3_test4/')
--cmd:option('-residual_blocks', 5)--16->8->5
cmd:option('-deconvolution_type','sub_pixel','sub_pixel|fullconvolution')--tarin3.lua change to resiza
cmd:option('-debug', true)--show every loss when train
-- Super-resolution options
cmd:option('-loss', 'percep', 'pixel|percep')--percep
cmd:option('-percep_layer', 'conv5_4', 'conv2_2|conv5_4')--con54
cmd:option('-percep_model', './models/VGG19.t7')
cmd:option('-use_tanh',true)--true?
-- Optimization
cmd:option('-num_epoch', 15)--100 is too much.change to 30/20
cmd:option('-batch_size',16)--origin is 16,too much for the computer.change it to 10
cmd:option('-learning_rate', 0.001)--train3.lua  decrease it when train
...
function main()
	...
	local dtype, use_cudnn = utils_fast.setup_gpu(opt.gpu, opt.backend, opt.use_cudnn)
	...
	if opt.resume_from_checkpoint ~= '' then
		...
		model = models_fast.build_model(opt):type(dtype)
	end
	...
	for epoch = opt.resume_epoch + 1, opt.num_epoch do
     ...
     if epoch % opt.checkpoint_every == 0 then
			...
			-- Save log ,Save a JSON checkpoint
                 	local log = {opt = opt, 
