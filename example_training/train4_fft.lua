...
-- Generic options
cmd:option('-h5_file','/mnt/lvm/xiaojie/imagenet-val-192_test2.h5')
cmd:option('-val_img','./imgs/comic_input.bmp')
cmd:option('-val_output','./train4_fft_test2/')
...
cmd:option('-checkpoint_name', './checkpoint/checkpoint_128')
...
function main()
	...
	if opt.resume_from_checkpoint ~= '' then
		...
		model = models_new.build_model(opt):type(dtype)
	end
...
local function f(x)
		...
		elseif opt.loss == 'percep' then
			...
			grad_out = percep_grad_in_:mul(255.0):index(2, torch.LongTensor{3,2,1})
		end
