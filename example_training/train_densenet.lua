...
cmd:option('-val_output','./checkpoints/checkpoint_densenet40_pixel/')
...
cmd:option('-checkpoint_name', './checkpoints/checkpoint_densenet40_pixel/densenet40_pixel')
...
function main()
	...
	if opt.resume_from_checkpoint ~= '' then
		...
		model = models_densenet.build_model(opt):type(dtype)
	end
