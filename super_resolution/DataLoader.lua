...
function DataLoader:__init(opt)
	...
	local train_edge_size = self.h5_file:read(string.format('%s_edge',self.image_paths.train)):dataspaceSize()
	...
	local val_edge_size = self.h5_file:read(string.format('%s_edge',self.image_paths.val)):dataspaceSize()
	...
	self.num_channels_edge=train_edge_size[2]
...
function DataLoader:getBatch(split)
	...
	local edge_images = self.h5_file:read(string.format('%s_edge',path)):partial(
						{start_idx, end_idx},
						{1, self.num_channels_edge},
						{1, self.label_image_height},
						{1, self.label_image_width}):float()
	if split == 'train' and self.random_flip then
		data_images, label_images, edge_images = dataprocess.random_flip_3(data_images,label_images,edge_images)
	end
	self.split_idxs[split] = end_idx + 1
	if self.split_idxs[split] > self.split_sizes[split] then
		self.split_idxs[split] = 1
	end
	return data_images, label_images, edge_images
end