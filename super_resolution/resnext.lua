...
local function createModel(opt)
   ...
   local function resnet_bottleneck(n, stride)
      local nInputPlane = iChannels--64
      iChannels = n * 4--16*4

      local s = nn.Sequential()
      s:add(Convolution(nInputPlane,n,1,1,1,1,0,0))--64,16
      s:add(SBatchNorm(n))
      s:add(ReLU(true))
      s:add(Convolution(n,n,3,3,stride,stride,1,1))--16,16
      s:add(SBatchNorm(n))
      s:add(ReLU(true))
      s:add(Convolution(n,n*4,1,1,1,1,0,0))--16,64
      s:add(SBatchNorm(n * 4))--64

      return nn.Sequential()
         :add(nn.ConcatTable()
            :add(s)
            :add(shortcut(nInputPlane, n * 4, stride)))--64,64
         :add(nn.CAddTable(true))
         :add(ReLU(true))
   end
   
   -- The aggregated residual transformation bottleneck layer, Form (B)   拆分
   local function split(nInputPlane, d, c, stride)--d:      c:
   ...
   local function resnext_bottleneck_B(n, stride)
      ...
      local D = math.floor(n * (opt.baseWidth/64))--baseWidth=4
      local C = opt.cardinality--cardinality=32
